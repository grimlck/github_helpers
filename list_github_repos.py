...
def get_repositories(github_user):
    ...
    The return value is a list of dictionaries which contain the
    repository data. Available keys can be found here:
...
def main():
    ...
    #build argument parser, takes two arguments:
    #GitHub username and number of repositories to show
    ap = argparse.ArgumentParser(
        description="List GitHub repositories by user"
    )
    ap.add_argument("username", type=str, help="GitHub username")
    ap.add_argument("-n", dest="show_only",
                    metavar="number",
                    type=int,
                    help="Show only first n repositories",
                    default=-1)
    args = ap.parse_args()
