...
def request_token(username, password, scopes=[""], note=""):
    ...
    if username and password and error == 0:
        ...
        base64string = base64.encodestring(str(username)
                                           + ":"
        ...
        try:
            ...
            # as the response comes in JSON format, we need to deserialize it
            # to a Python object
            result = json.loads('\n'.join(result.readlines()))

            if result['token'] and isinstance(result, dict):
        ...
        return [1,
                {"message": "Wrong scopes or username/password not specified"}]
...
def repository_exists(username, repository_name):
    ...
    request = urllib2.Request(
        "https://api.github.com/repos/"+str(username)+"/"+str(repository_name))
    ...
    if token and repository_name:
        ...
        if auto_init is True:
            ...
            if gitignore_template:
                create_options += ', "gitignore_template": "'
                + str(gitignore_template)+'"'
        ...
        try:
            ...
            if e.getcode() == 500:
                response = {"message": "Repository created, but with "
                            + str(e)+": "
                            + json.loads("\n".join(e.readlines()))['message']}
            else:
...
def main():
    ...
    ap.add_argument('-n', dest='repo_name', metavar='Repository name',
                    type=str, required=True,
                    help='Name of the new GitHub repository')
    ap.add_argument('-d', dest='desc', metavar='Description',
                    type=str, required=False,
                    help='Description for the new repository')
    ap.add_argument('-a', action='store_true',
                    help='Initialize repository with empty README.md')
    ap.add_argument('-g', dest='lang', metavar='Language or Platform',
                    type=str, required=False,
                    help='''Initialize repository with
